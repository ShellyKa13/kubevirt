	...
	case admissionv1.Create:
		...
		if vmBackup.Spec.Source == nil && vmBackup.Spec.BackupTracker == nil {
			causes = append(causes, metav1.StatusCause{
				...
				Message: "must specify either backup source or backup tracker",
				Field:   sourceField.String(),
			})
		} else if vmBackup.Spec.Source != nil && vmBackup.Spec.BackupTracker != nil &&
			!equality.Semantic.DeepEqual(vmBackup.Spec.Source, vmBackup.Spec.BackupTracker.Spec.Source) {
			causes = append(causes, metav1.StatusCause{
				Type:    metav1.CauseTypeFieldValueNotFound,
				Message: "in case both source and backupTracker are specified, the source in the backupTracker must equal the backup source",
				Field:   sourceField.String(),
			})
		}
...
func (admitter *VMBackupAdmitter) validateSingalBackup(vmBackup *backupv1.VirtualMachineBackup, namespace string, causes []metav1.StatusCause) ([]metav1.StatusCause, error) {
	...
	for _, obj := range objects {
		...
		// TODO: check sources and backupTracker sources are equal
		if equality.Semantic.DeepEqual(vmbackup2.Spec.Source, vmBackup.Spec.Source) &&
			// TODO: add check vmbackup2 !completed
			(vmbackup2.Status == nil) {
			causes = append(causes, metav1.StatusCause{
				Type:    metav1.CauseTypeFieldValueInvalid,
				Message: fmt.Sprintf("VirtualMachineBackup %q in progress", vmbackup2.Name),
				Field:   sourceField.String(),
			})
			return causes, nil
		}
