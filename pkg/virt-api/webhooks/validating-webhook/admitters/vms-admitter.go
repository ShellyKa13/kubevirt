...
func (admitter *VMsAdmitter) validateVolumeRequests(ctx context.Context, vm *v1.VirtualMachine) ([]metav1.StatusCause, error) {
	...
	causes := storageAdmitters.ValidateVMISpecStorageFields(k8sfield.NewPath("spec", "template", "spec"), newSpec, admitter.ClusterConfig)
	if len(causes) > 0 {
		return causes, nil
	}
	...
	if vmiExists {
		causes := storageAdmitters.ValidateVMISpecStorageFields(k8sfield.NewPath("spec"), &vmi.Spec, admitter.ClusterConfig)
		if len(causes) > 0 {
			return causes, nil
		}
